...
INSERT INTO cotas (grupo_id, cliente_id, numero_cota, status, version)
SELECT g.id, c.id, 999, 'ATIVA', 0
FROM grupos g, clientes c
WHERE g.codigo = 'GRUPO_PLD_FT' AND c.cpf_cnpj = '98765432100'
ON CONFLICT DO NOTHING;

INSERT INTO assembleias (grupo_id, data_assembleia, tipo, status, version)
SELECT g.id, CURRENT_DATE - 5, 'ORDINARIA', 'REALIZADA', 0
FROM grupos g
WHERE g.codigo = 'GRUPO_PLD_FT'
ON CONFLICT DO NOTHING;
